        ...
        <tr v-for="intrare in llist">
          ...
          <td>
            <input
              ...
              :id="intrare.id"
              @click="primitService"
              :disabled="intrare.status != 'Expediat @ ' + intrare.nume_vapoint"
              class="checkbox"
            >
          </td>
          <td>
            <input
              ...
              :id="intrare.id"
              @click="primitService"
              :disabled="intrare.status != 'Primit @ Service' || intrare.remediat == 0"
              class="checkbox"
            >
          </td>
          <td>
            <i
              :id="intrare.id"
              :telefon="intrare.telefon_client"
    ...
    <div class="columns" v-if="show == 'produse'">
        ...
        <div class="card">
          ...
          <footer class="card-footer">
            <p class="card-footer-item">
              <span>Telefon: {{telefon_client}}</span>
            </p>
            <p class="card-footer-item">
      ...
      <div class="column">
        <div class="card">
          ...
          <footer class="card-footer">
            <p class="card-footer-item">
              <span>Telefon: {{telefon_vapoint}}</span>
            </p>
            <p class="card-footer-item">
              <span>Email: {{email_vapoint}}</span>
            </p>
          </footer>
        </div>
      </div>
...
export default {
  ...
  methods: {
    getProduse: function(event) {
      ...
      axios
        .post("admin/getVapoints", { id: this.id_vapoint })
        .then(response => {
      ...
      axios
        .post("admin/getProduseIntrare", { id: this.id_intrare })
        .then(response => {
    ...
    showIntrari: function(event) {
      axios
        .post("admin/getService", { vap: this.id_vapoint })
        .then(response => {
    ...
    primitService: function(event) {
      ...
      axios
        .post("garantii/primit_service", { id: id, stat: stat })
        .then(response => {
          this.llist = response.data;
        });
    },
    rezolvat: function(event) {
      ...
      axios
        .post("garantii/rezolvat", { id: this.id_intrare, text: text })
        .then(response => {
          this.errors = "";
        })
        .catch(error => {
